...
class ModelArguments:
    ...
    use_wi: Optional[int] = field(
        default=None,
        metadata={"help": "Whether to use word importance metrics."
        },
    )
...
def main():
    ...
    def preprocess_function(examples):
        ...
        # WI calculation
        if model_args.use_wi:
            temp = list(map(ids_to_string,tokenized_examples['input_ids']))
            sents = []
            for sen in temp:
                sents.append(' '.join(sen))
            tfidfvec,vec = get_tfidf_vector(sents,model_args.use_wi)
            tfidf_ids = []
            #print(tokenized_examples['input_ids'][:4])
            ...
            for i,sen in enumerate(sents):
                ...
                tmp += [0]*(128-len(tmp))
                assert len(tmp)==128, tmp
                tfidf_ids.append(tmp)
            
            tokenized_examples['tfidf_ids'] = tfidf_ids
